<section id="contact" class="section-indentation about">
    <div class="container">
        <h2 class="about-title">Контактна інформація</h2>
        <div class="contact-wrap">
        <div class="contact-item">
            <div class="contact-box">
                <svg class="contact-icon" width="20" height="20" aria-label="city">
                    <use href="./assets/img/symbol-defs.svg#icon-city"></use>
                </svg>
                <span class="contact-text">Вул. Льва Толстого, 7, Харків, 61000, Україна</span>
            </div>
            <div class="contact-box">
                <svg class="contact-icon" width="20" height="20" aria-label="phone">
                    <use href="./assets/img/symbol-defs.svg#icon-phone"></use>
                </svg>
                <span class="contact-text">+380 57 765 43 21</span>
            </div>
            <div class="contact-box">
                <svg class="contact-icon" width="20" height="20" aria-label="email">
                    <use href="./assets/img/symbol-defs.svg#icon-email"></use>
                </svg>
                <a class="contact-link" href="mailto:contact@company.com">contact@company.com</a>
            </div>
            <div class="contact-box">
                <svg class="contact-icon" width="20" height="20" aria-label="location">
                    <use href="./assets/img/symbol-defs.svg#icon-map"></use>
                </svg>
                <a class="contact-link" href="https://www.google.com/maps?q=Харків,+вул.+Льва+Толстого,+7" target="_blank">Перейти</a>
            </div>
        </div>
        <div><img src="./assets/img/contacts.png" alt="paty" width="520" ></div>
    </div>
    </div>
</section>
